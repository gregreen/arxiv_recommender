...
from arxiv_lib.config import (
    ...
    SEARCH_SCORE_SHASH_MU,
    SEARCH_SCORE_SHASH_SIGMA,
    SEARCH_SCORE_SHASH_DELTA,
    SEARCH_SCORE_SHASH_EPSILON,
)
...
def cosine_to_search_score(cosine: np.ndarray) -> np.ndarray:
    """Convert cosine similarity to a calibrated search score.

    Evaluates the SHASH (Sinh-Arcsinh) distribution CDF to produce
    ln P(random similarity > X).  Inputs are clipped to [-1, 1] to
    guard against floating-point overshoot.

    Returns an ndarray of the same shape as *cosine*, with scores in
    (-∞, 0]. The score represents the log-probability of the given paper
    being in the top 0.5% of most relevant arXiv papers.
    """
    x = np.clip(cosine, -1.0, 1.0)
    w = (x - SEARCH_SCORE_SHASH_MU) / SEARCH_SCORE_SHASH_SIGMA
    C = SEARCH_SCORE_SHASH_DELTA * np.arcsinh(w) - SEARCH_SCORE_SHASH_EPSILON
    C = np.clip(C, -250.0, 250.0) # Clip |C| to prevent sinh overflow during scoring
    logcdf = _norm.logcdf(np.sinh(C))
    return logcdf
...
def search_papers(
    ...
    sims   = 3 * cosine_to_search_score(_cosine_similarity(q, matrix))
    # Multiplied by 3 because of how scores are shown to user in UI
