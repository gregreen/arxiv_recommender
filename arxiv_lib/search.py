...
# ---------------------------------------------------------------------------
# Search score calibration (percentile-based scoring)
# ---------------------------------------------------------------------------
# Lookup table mapping cosine similarity → percentile rank among random papers.
# Generated by: python experiments/interpret_search_score_statistics.py
# (which reads experiments/search_score_calibration.json produced by
#  python experiments/search_score_calibration.py)
#
# The final score is ln P(random similarity > X) = ln(clip(percentile/100, 1e-12, 1))
# giving 0 at the 100th percentile and ≈ −27.6 at the 0th percentile.
_SEARCH_SCORE_PERCENTILES_FILE = os.path.join(BASE_DIR, "search_score_percentiles.json")


def _load_search_score_percentiles(path: str) -> tuple[np.ndarray, np.ndarray] | None:
    """Load the (cosine_knots, percentile_fraction_knots) arrays from *path*.

    Returns None if the file is absent, triggering the legacy scoring fallback.
    """
    if not os.path.exists(path):
        return None
    with open(path) as f:
        data = json.load(f)
    cosine_knots       = np.array(data["cosine_similarity"], dtype=np.float64)
    pct_fraction_knots = np.array(data["percentile"],        dtype=np.float64) / 100.0
    return cosine_knots, pct_fraction_knots


_percentile_result = _load_search_score_percentiles(_SEARCH_SCORE_PERCENTILES_FILE)
if _percentile_result is None:
    _log.warning(
        "Search score percentile table not found at %r; "
        "falling back to legacy 3·log(cosine) scoring. "
        "Run: python experiments/search_score_calibration.py",
        _SEARCH_SCORE_PERCENTILES_FILE,
    )
    _SEARCH_SCORE_COSINE_KNOTS = None
    _SEARCH_SCORE_PCT_KNOTS    = None
else:
    _SEARCH_SCORE_COSINE_KNOTS, _SEARCH_SCORE_PCT_KNOTS = _percentile_result


def cosine_to_search_score(cosine: np.ndarray) -> np.ndarray:
    """Convert cosine similarity to a calibrated search score.

    If the percentile lookup table is available, returns
    ln P(random similarity > X) = ln(clip(pct/100, 1e-12, 1)), with scores
    in (−∞, 0].  Falls back to the legacy 3·log(clip(cosine, 1e-12, 1))
    formula when the table has not yet been generated.
    """
    if _SEARCH_SCORE_COSINE_KNOTS is not None:
        fraction = np.interp(cosine, _SEARCH_SCORE_COSINE_KNOTS, _SEARCH_SCORE_PCT_KNOTS)
        return np.log(np.clip(fraction, 1e-12, 1.0))
    return 3.0 * np.log(np.clip(cosine, 1e-12, 1.0))
